...
Significant rewrite of `fourtwenty-block-tracker`. Primary reason was optimizing network IO.
...
Events now only return the block number. Internal polling is done via `fourtwenty_blockNumber`.
...
- module includes dist:
  - bundle: `dist/FourtwentyBlockTracker.js`
  - es5 source: `dist/es5/`
- fixes `awaitCurrentBlock` return value
- `lib` renamed to `src`
- `fourtwenty-block-tracker` is now a normal `EventEmitter`, does not provide a callback to event handlers
